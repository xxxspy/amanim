// import { SpriteText2D, textAlign } from '../../../libs/three-text2d.min.js'
import { MathExpression } from './MathExpression';
import { MathGraphMaterial } from './MathGraphShader';
import { MathCurveMaterial } from './MathCurveShader';
const SpriteText2D = THREE_Text2D.SpriteText2D;
...
AFRAME.registerComponent('graph', {
    ...
    update: function (oldData) {
        ...
        if((this.data.width != oldData.width)||(this.data.height != oldData.height)){
            this.graph.geometry = new THREE.PlaneBufferGeometry(this.data.width, this.data.height, 300, 300);
            this.graph.geometry.scale(2, 2, 2)
        }
    ...
    createSurface: function (expression) {
        new THREE.BufferGeometry();
        const graphGeometry = new THREE.PlaneBufferGeometry(this.data.width, this.data.height, 300, 300);
        graphGeometry.scale(2, 2, 2);
        const graphMat = new MathGraphMaterial(expression);
        // const graph = new THREE.Mesh(graphGeometry, new THREE.MeshBasicMaterial( {color: 0xffff00, side: THREE.DoubleSide} ));
